/**
 * PSmilesToGraphStream (Particle SMILES to GraphStream)
...
package de.gnwi.psmilestographstream;
...
/**
 * Class to generate GraphStream objects from PSmiles objects.
 * 
 * @author Mirco Daniel, Jonas Schaub
 */
public class PSmilesToGraphStream {
     ...
     * {@link de.gnwi.psmiles.PSmiles} object.
     * Also adds special nodes to the particles with [START]  and [END] tags.
     * 
     * @param aGraph a GraphStream graph that should display the 
     * PSMILES structure
     * @param aPSmiles a {@link de.gnwi.psmiles.PSmiles} object that should be 
     * displayed in the graph
     * @param aPartNumber the part of the {@link de.gnwi.psmiles.PSmiles} 
     * object to be displayed
     * @throws IllegalArgumentException if aGraph or aPSmiles is 'null' or aPartNumber
     * is invalid
     * @author Jonas Schaub
     */
    private void buildNodesAndEdges(PSmiles aPSmiles, Graph aGraph, int aPartNumber) {
        if (aPSmiles == null) { throw new IllegalArgumentException("aPSmiles (instance of class PSmiles) is null."); }
        if (aGraph == null) { throw new IllegalArgumentException("aGraph (instance of class Graph) is null."); }
        if (aPSmiles.getErrorMessage() != null) {
            return;
        }
        int tmpPartsNumber = aPSmiles.getPartsOfPSmiles().length;
        if (aPartNumber > tmpPartsNumber || aPartNumber < 0) {
            throw new IllegalArgumentException("The PSmiles object has got no "
                    + aPartNumber + "th part!");
        }
        ...
        tmpParticleNumber = aPSmiles.getPartsOfPSmiles()[aPartNumber].getInnerParticles().length;
        tmpNodeID[aPartNumber] = new String[tmpParticleNumber];
        for (int j = 0; j < tmpParticleNumber; j++) {
            // Build nodes
            tmpNodeIDBuilder = new StringBuilder(aPSmiles.getPartsOfPSmiles()[aPartNumber].getInnerParticles()[j]);
            ...
            tmpNode.setAttribute("ui.class", aPSmiles.getPartsOfPSmiles()[aPartNumber].getInnerParticles()[j]);
            if (!this.isFullParticleNameDisplay) {
                if (aPSmiles.getPartsOfPSmiles()[aPartNumber].getInnerBackboneIndices()[j] != 0) {
                    tmpLabel = aPSmiles.getPartsOfPSmiles()[aPartNumber].getInnerParticles()[j].replaceAll("([A-Z][a-zA-Z0-9])[a-zA-Z0-9]{2,8}", "$1.");
                } else {
                    tmpLabel =  aPSmiles.getPartsOfPSmiles()[aPartNumber].getInnerParticles()[j].replaceAll("([A-Z][a-zA-Z0-9])[a-zA-Z0-9]{2,6}([a-zA-Z0-9][a-zA-Z0-9])", "$1.$2");
                } 
            } else {
                tmpLabel = aPSmiles.getPartsOfPSmiles()[aPartNumber].getInnerParticles()[j];
            }
            if (aPSmiles.getPartsOfPSmiles()[aPartNumber].getInnerBackboneIndices()[j] != 0) {
                    tmpLabel = tmpLabel + "'" + aPSmiles.getPartsOfPSmiles()[aPartNumber].getInnerBackboneIndices()[j] + "'";
            }
        ...
        if (tmpParticleNumber > 1) {
            int[][] tmpAdjacentArray = this.convertToNoneRedundantAdjacentArray(
                    aPSmiles.getPartsOfPSmiles()[aPartNumber].getAdjacentArray());
        ...
        if (aPSmiles.getPartsOfPSmiles()[aPartNumber].getStartParticleIndex() > -1) {
            int tmpStartIndex = aPSmiles.getPartsOfPSmiles()[aPartNumber].getStartParticleIndex();
            int tmpEndIndex = aPSmiles.getPartsOfPSmiles()[aPartNumber].getEndParticleIndex();
    ...
    /**
     * Convert a PSmiles object to match GraphStream Graph object (more parts will separated in more Graph objects)
     * 
     * @param aPSmiles PSMILES object
     * @return GraphStream Graph object(s)
     * @author Mirco Daniel
     */
    public Graph[] convertPSmilesToStreamGraph(PSmiles aPSmiles) {
        if (aPSmiles == null || aPSmiles.getErrorMessage() != null) {
            return new Graph[0];
        }
        int tmpPartsNumber = aPSmiles.getPartsOfPSmiles().length;
        int tmpParticleNumber;
        ...
        for (int i = 0; i < tmpPartsNumber; i++) {
            ...
            tmpParticleNumber = aPSmiles.getPartsOfPSmiles()[i].getInnerParticles().length;
            tmpNodeID[i] = new String[tmpParticleNumber];
            for (int j = 0; j < tmpParticleNumber; j++) {
                // Build nodes
                tmpNodeIDBuilder = new StringBuilder(aPSmiles.getPartsOfPSmiles()[i].getInnerParticles()[j]);
                tmpNodeIDBuilder.append('(');
                ...
                tmpNode.setAttribute("ui.class", aPSmiles.getPartsOfPSmiles()[i].getInnerParticles()[j]);
                if (aPSmiles.getPartsOfPSmiles()[i].getInnerParticles()[j].length() > 5) {
                    tmpLabel =  aPSmiles.getPartsOfPSmiles()[i].getInnerParticles()[j].substring(0, 3) + "..";
                } else {
                    tmpLabel = aPSmiles.getPartsOfPSmiles()[i].getInnerParticles()[j];
                }
                if (aPSmiles.getPartsOfPSmiles()[i].getInnerBackboneIndices()[j] != 0) {
                    tmpLabel = tmpLabel + "'" + aPSmiles.getPartsOfPSmiles()[i].getInnerBackboneIndices()[j] + "'";
                }
                tmpNode.addAttribute("ui.label", tmpLabel);
            }
            // Build edges
            if (tmpParticleNumber > 1) {
                int[][] tmpAdjacentArray = this.convertToNoneRedundantAdjacentArray(aPSmiles.getPartsOfPSmiles()[i].getAdjacentArray());
                for (int j = 0; j < tmpAdjacentArray.length; j++) {
                    if (tmpAdjacentArray[j] != null) {
                        for (int k = 0; k < tmpAdjacentArray[j].length; k++) {
        ...
        for (int i = 0; i < tmpPartsNumber; i++) {
            if (aPSmiles.getPartsOfPSmiles()[i].getStartParticleIndex() > -1) {
                int tmpStartIndex = aPSmiles.getPartsOfPSmiles()[i].getStartParticleIndex();
                int tmpEndIndex = aPSmiles.getPartsOfPSmiles()[i].getEndParticleIndex();
     ...
     * {@link de.gnwi.psmilestographstream.Constants.Shape} enum object; If 'null'
     ...
     * in {@link de.gnwi.psmilestographstream.Constants.Shape}
     * @author Jonas Schaub
     */
     ...
     * {@link de.gnwi.psmiles.PSmiles} object. The graph is not reinstantiated but cleared and then
     * filled with elements and attributes according to the {@link de.gnwi.psmiles.PSmiles} object so that
     * a GraphStream {@link org.graphstream.ui.view.Viewer} observing the 
     * graph does not loose connection to it.
     * The different parts of the {@link de.gnwi.psmiles.PSmiles} object are all combined in one graph
     * (but unconnected). If the {@link de.gnwi.psmiles.PSmiles}' error message is not 'null' the graph
     ...
     * should display the {@link de.gnwi.psmiles.PSmiles} structure
     * @param aPSmiles a {@link de.gnwi.psmiles.PSmiles} object that should be displayed in the graph
     * @throws IllegalArgumentException if aGraph or aPSmiles is 'null'
     * @author Jonas Schaub
     */
    public void updatePsmilesGraph(Graph aGraph, PSmiles aPSmiles) throws IllegalArgumentException {
        if (aPSmiles == null) { throw new IllegalArgumentException("aPSmiles (instance of class PSmiles) is null."); }
        if (aGraph == null) { throw new IllegalArgumentException("aGraph (instance of class Graph) is null."); }
        aGraph.clear();
        if (aPSmiles.getErrorMessage() != null) {
        ...
        for (int i = 0; i < tmpPartsNumber; i++) {
            this.buildNodesAndEdges(aPSmiles, aGraph, i);
        }
     ...
     * If the {@link de.gnwi.psmiles.PSmiles}' error message is not 'null' the graph
     * is only cleared. Node color, node shape, edge weight, node weight and
     * node size are set according to the matching object fields if they
     * have been updated.
     *
     * @param aGraph a GraphStream {@link org.graphstream.graph.Graph} 
     * that should display the {@link de.gnwi.psmiles.PSmiles} structure
     * @param aPSmiles a {@link de.gnwi.psmiles.PSmiles} object that should be displayed in the graph
     * @param aPartNumber the part of the {@link de.gnwi.psmiles.PSmiles} object to be displayed
     * @throws IllegalArgumentException if aGraph or aPSmiles is 'null' or aPartNumber
     * is invalid
     * @author Jonas Schaub
     */
    public void updatePsmilesGraph(Graph aGraph, PSmiles aPSmiles, int aPartNumber)
        ...
        this.addAttributes(aGraph);
        this.buildNodesAndEdges(aPSmiles, aGraph, aPartNumber);
        if (this.isNodeColorUpdated) {
            for (Node tmpNode : aGraph) {
                tmpNode.addAttribute("ui.color", this.currentNodeColor);
            }
