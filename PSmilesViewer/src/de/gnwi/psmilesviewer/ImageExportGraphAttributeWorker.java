 ...
 * {@link de.gnwi.psmiles.PSmiles} graph as an image file by adding a
...
public class ImageExportGraphAttributeWorker extends SwingWorker {
    ...
    /**
     * The PSmiles graph that should be exported.
     */
     ...
     * @param aMainFrame The {@link de.gnwi.psmilesviewer.MainFrame} where to 
     * write a message at success or failure
     * @param aUserPreferences to draw the color of the message in 
     * the {@link de.gnwi.psmilesviewer.MainFrame} from
     ...
     * the graph has no nodes or the {@link de.gnwi.psmilesviewer.UserPreferences}'
     * 'valid' color is 'null'
     */
    public ImageExportGraphAttributeWorker(File anImageFile,
        ...
        if (aSingleGraph.getNodeCount() == 0) {
            throw new IllegalArgumentException("The PSmiles graph does not have any nodes!");
        }
    ...
    /**
     * Exports the given {@link de.gnwi.psmiles.PSmiles} graph by adding a
     * 'ui.screenshot' attribute to it that the GraphStream Viewer intercepts causing it
     * to export the graph (the visible section if it has been dragged and/or 
     * zoomed). The thread sleeps for 
     * {@link de.gnwi.psmilesviewer.Constants#WAITING_TIME_MILLIS_FOR_IMAGE_EXPORT_BY_GRAPHATTRIBUTE} divided by
     * {@link de.gnwi.psmilesviewer.Constants#WAITING_STEPS_FOR_IMAGE_EXPORT_BY_GRAPHATTRIBUTE}
     ...
     * {@link de.gnwi.psmilesviewer.Constants#WAITING_STEPS_FOR_IMAGE_EXPORT_BY_GRAPHATTRIBUTE} steps have been gone through.
