 ...
 * exemplary PSMILES structures and to load them into the 
 * PSmiles Viewer application.
 * 
 * @author Jonas Schaub
 */
public abstract class StructurePropertiesReader {
     ...
     */
    private static final String BUNDLE_NAME = "de.gnwi.psmilesviewer.Structures";
    ...
    /**
     * Returns the resource's PSMILES structure for the given key. Does not throw a
     * MissingResourceException if the string could not be found but returns 
     * "[PSmiles structure for the key " + aKey + "could not be found!]" instead.
     * 
     * @param aKey the PSMILES structure's key in the 
     * 'Structures.properties' file
     * @return the PSMILES structure linked to the given aKey
     */
    public static String getString(String aKey) {
        try {
            return RESOURCE_BUNDLE.getString(aKey).trim();
        } catch (MissingResourceException aMissingResourceException) {
            return "[PSmiles structure for the key " + aKey + "could not be found!]";
        }
    }
    //</editor-fold>
}
